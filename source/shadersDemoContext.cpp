...
void BeginPoints(float size) { s_context->beginPoints(size);  }
void DrawPoint(const Vec3& p, const Vec4& color) { s_context->drawPoint(p, color); }
void EndPoints() { s_context->endPoints();  }
...
{
  s_context->bindHydrographicShader(lightPos, lightTarget, lightTransform, shadowMap, bias, fogColor);
}
