  ...
  std::string basePath = p.substr(0, found);
    ...
    while (file)
    {
        ...
        else if (strcmp(buffer, "vt") == 0)
        {
            ...
            float u, v;
            file >> u >> v;
            //m->m_tex_coords.push_back(Vector2(u, v));
    ...
    ifstream mtlFile(basePath + '/' + MaterialFile);
    if (mtlFile)
    {
      while (mtlFile)
      {
        ...
        if (strcmp(buffer, "Ks") == 0)
        {
          ...
          float r, g, b;
          mtlFile >> r >> g >> b;
          m->Ks = Vec4(r, g, b, 1.0f);
        }
        if (strcmp(buffer, "map_Kd") == 0)
        {
          // difuse texture map
          std::string MapDiffuseFile;
          mtlFile >> MapDiffuseFile;
          m->map_Kd = basePath + '/' + MapDiffuseFile;
        }
      }
    }
    return m;
